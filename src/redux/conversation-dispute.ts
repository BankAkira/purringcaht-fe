...
import { setLatestMessageDisputeAction } from './message-dispute';

const log = new Logger('conversation-dispute-redux');
...
const fetcher = getMyConversationsDisputeApi;
...
export const updatePayloadByIdAction =
  ...
  async (dispatch: StoreDispatch, getState: StoreGetState) => {
    try {
      const { results } = getState().conversationDispute;
      const { conversation } = getState().messageDispute;
      ...
      if (targetConversation) {
        ...
        const newPayload = await getDisputeByConversationId(conversationId);
        if (newPayload) {
...
export const fetchMessages =
  ...
  (
    ...
    const { results, isInitializing } = getState().conversationDispute;
            ...
            onValue(listenRef, snapshot => {
              if (snapshot.exists()) {
                ...
                if (!isEmpty(val)) {
                  ...
                  dispatch(setLatestMessageDisputeAction(val));
                }
              }
              resolveInner();
            });
...
export const fetchDeletedMessages =
  ...
  async (dispatch: StoreDispatch, getState: StoreGetState): Promise<void> => {
    const { database } = getState().firebase;
    const { deletedResults, isInitializing } = getState().conversationDispute;
    return new Promise(() => {
