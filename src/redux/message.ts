...
import { resetUnreadCountByIdAction, setIsFetchFirebase } from './conversation';
import {
...
export const { initializeChatSchemeAction, setChatSecret } =
...
export const initializeMessageAction =
  ...
  async (dispatch: StoreDispatch, getState: StoreGetState) => {
    try {
      const { queryParams } = getState().message;
...
export const sendMessageAction =
  (messageBody: SendMessageBody, officialSender?: User) =>
  async (dispatch: StoreDispatch, getState: StoreGetState) => {
    ...
    try {
      ...
      const sendingMessage: MessagePayload = {
        ...
        createdAt: '',
        updatedAt: '',
        senderId: officialSender ? officialSender.id : user.id,
      ...
      if (officialSender) {
        await sendConversationMessageBotApi(conversation.id, messageBody);
      } else {
      ...
      if (
        messageBody.contentType === MessageContentType.IMAGE ||
        messageBody.contentType === MessageContentType.VIDEO
      ) {
        dispatch(loadMediaResultsAction({ page: 1 }));
      }
      if (messageBody.contentType === MessageContentType.FILE) {
...
export const loadMoreMessageAction =
  () => async (dispatch: StoreDispatch, getState: StoreGetState) => {
    try {
      const { conversation, messages, queryParams } = getState().message;
...
export const setLatestMessageAction =
