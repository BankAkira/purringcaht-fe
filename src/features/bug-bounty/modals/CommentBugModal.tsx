...
import {
  ...
  ConfirmReportBugStatus,
} from '../../../type/bug-bounty.ts';

// redux
import { useDispatch } from '../../../redux';
import { addComment } from '../../../redux/bug-bounty';
...
// APIs
import { createConfirmReportBugApi } from '../../../rest-api/bug-bounty';

const log = new Logger('CommentBugModal');

type Props = {
  openModal: boolean;
  onCloseModal: () => void;
  onCommentBug: (data: ConfirmReportBugPayload) => Promise<void>;
  reportBugId: string;
};
...
export default function CommentBugModal({
  openModal,
  onCloseModal,
  onCommentBug,
  reportBugId,
  ...
  const [formData, setFormData] = useState<ConfirmReportBugPayload>({
    reportBugId,
    status: '' as ConfirmReportBugStatus,
    detail: '',
    files: [],
  });
  ...
  // StatusChange //
  const handleStatusChange = (status: ConfirmReportBugStatus) => {
  ...
  const handleConfirmCommentBug = async () => {
    try {
      loadingStart();
      const response = await createConfirmReportBugApi(formData);
      if (response) {
        log.debug('Comment bug report created successfully', response);
        dispatch(addComment(response));
        await onCommentBug(formData);
    ...
    <>
      {isLoading ? (
        ...
        <Modal
          ...
          <Modal.Header>Make Comments</Modal.Header>
          <form onSubmit={handleSubmit}>
                ...
                <div className="flex flex-col w-full gap-2">
                  <CustomRadio>
                    <div className="flex flex-row gap-4">
                      <div className="flex items-center gap-2 border rounded-[1rem] p-2 custom-radio">
                        <Radio
                          id="agree"
                          name="status"
                          value="AGREE"
                          checked={formData.status === 'AGREE'}
                          onChange={() => handleStatusChange('AGREE')}
                        />
                      ...
                      <div className="flex items-center gap-2 border rounded-[1rem] p-2 custom-radio">
                        <Radio
                          ...
                          name="status"
                          value="DISAGREE"
                          checked={formData.status === 'DISAGREE'}
                          onChange={() => handleStatusChange('DISAGREE')}
                        />
            ...
            <Modal.Footer className="justify-end">
              ...
              <Button
                disabled={!formData.status || !formData.detail}
                pill
              ...
              <ConfirmModal
                ...
                color="red"
                description="Are you sure you want to submit this comments?"
              />
              <ErrorModal
